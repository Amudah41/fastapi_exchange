...
def test_login(overrides_get_db):
    # correct user data
    response = client.post(
        url="/auth/login/", data={"username": "username_test", "password": "qwerty123"}
    )
    assert response.status_code == 200
    data_json = response.json()
    ...
    # incorrect user data
    response = client.post(
        url="/auth/login/", data={"username": "username_test", "password": "123"}
    )
...
def test_about_me(overrides_get_db):
    ...
    response = client.get(url="/auth/about_me/", headers=headers)
    assert response.status_code == 200
